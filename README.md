...
- Copy comments: the note list handed back as text, selected for Ctrl+C where the clipboard is blocked

Concept review ships a single self-contained `preview.html` (target 850,000 bytes, strictly below 1,000,000); CAD files, records and review PNGs stay private. The Three.js mesh is for design review, not measurement. Exact gaps, intersections, clearances and exported geometry remain the responsibility of the OCP CAD pipeline. Interactive preview requires the pinned Three.js CDN, ES modules, WebGL and `DecompressionStream`; a failure message appears when loading fails.
...
| Tag | State |
|---|---|
| [`v1.2.3`](../../tree/v1.2.3) | Current. Standard GH-201-B clamp mount plate, a cross-support check so no upright is left unbraced, a single gzip-compressed `preview.html` as the only concept artifact, and the restored shop-stock nester with single-stroke etch. Replaces the v1.2.2 snapshot button with Copy comments. |
| [`v1.2.2`](../../tree/v1.2.2) | Click-to-comment pins and a snapshot button in the datum-scheme review and the concept preview. Comments are review notes only and never edit the spec. |
