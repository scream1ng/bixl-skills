...
Run `python scripts/validate_delivery.py DELIVERY`. It checks types, required checks, status consistency, revision/fingerprint agreement, actual file readability, closed DXF loops, STEP component presence, PNG validity and export hashes. Structural validation does not establish physical manufacturing readiness. The build exits 1 on failed checks and 2 on incomplete delivery; exit 0 indicates a complete concept package whose engineering status must still be read.
