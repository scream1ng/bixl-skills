...
- If a DXF is not relevant to a block fixture, explain the exception and deliver the requested manufacturing drawing or model format instead. The request for block construction permits this format exception when there is no laser-cut content; record it and validate with `scripts/validate_delivery.py --allow-no-dxf <DELIVERY directory>`.
